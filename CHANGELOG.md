...
## [Phase D — Network IDS] — 2026-06-25 — COMPLETE

### Added
- Suricata `7.0.8_5` on pfSense LAN (vtnet1) in IDS mode (Block Offenders off, Detect-Engine
  Profile Medium, EVE JSON on); rulesets ETOpen Emerging Threats + Feodo Tracker Botnet C2 IP
  + ABUSE.ch SSL Blacklist; attack-mapped rule categories enabled. First detection validated
  in the Suricata Alerts tab: a `testmynids.org` curl tripped GPL ATTACK_RESPONSE id check
  returned root (SID 2100498).
- Scoped-key file-pull pipeline shipping Suricata EVE JSON from pfSense to Wazuh:
  forced-command, restricted ed25519 SSH key (`cat` of the one EVE file, nothing else);
  `pull-eve.sh` incremental shipper (mark-and-append with rotation reset, dedup proven);
  per-minute cron; Wazuh `localfile` JSON ingestion.
- End-to-end validation: `wazuh-logtest` confirms decode + rule **86601**
  (`ids,suricata`, level 3); the Wazuh dashboard (`rule.groups:suricata`) shows **1,783
  Suricata alerts** indexed from the `bgranilloserver` agent.
- Runbooks: docs/runbooks/suricata-ids-setup.md, docs/runbooks/suricata-wazuh-integration.md.
- ADR-0007 — pull-based EVE ingestion (no agent on the firewall).

### Changed
- Abandoned the syslog EVE-forwarding approach — pfSense does not forward the Suricata
  `LOCAL1` facility, so EVE never left the firewall. Removed the dead syslog `<remote>`
  listener from `ossec.conf`. Reverted the Wazuh manager `<logall>` from `yes` (diagnostics)
  back to `no`.

### Known limitation
- Current dashboard alerts are all `SURICATA STREAM` TCP-anomaly events (baseline noise,
  partly self-generated by the SSH pull). Real attack-signature detection lands in Phase E.
  STREAM-noise suppression deferred (source-scoped per-SID, not a blanket mute).
